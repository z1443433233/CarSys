...
<body>
<div class="cBody">
    <form id="addForm" class="layui-form" action="${pageContext.request.contextPath}/admin/goodsModify">
        <div class="layui-form-item">
            <label class="layui-form-label">汽车编号</label>
            <div class="layui-input-block">
                <input type="text" name="id" value="${carMsg.id}" readonly required lay-verify="required" autocomplete="off" class="layui-input">
            </div>
        </div>
        <div class="layui-form-item">
            <label class="layui-form-label">汽车车型</label>
            <div class="layui-input-block">
                <input type="text" name="carName" value="${carMsg.carName}" required lay-verify="required" autocomplete="off" class="layui-input">
            </div>
        </div>
        <div class="layui-form-item">
            <label class="layui-form-label">商品图片url</label>
            <div class="layui-input-block">
                <input type="text" name="carPicture" value="${carMsg.carPicture}" required lay-verify="required" autocomplete="off" class="layui-input">
            </div>
        </div>
        <div class="layui-form-item">
            <label class="layui-form-label">租金/天</label>
            <div class="layui-input-block">
                <input type="number" name="carPrice" value="${carMsg.carPrice}" required lay-verify="required|number" autocomplete="off" class="layui-input">
            </div>
        </div>
        <div class="layui-form-item">
            <label class="layui-form-label">座位数</label>
            <div class="layui-input-block">
                <input type="number" name="carSitnum" value="${carMsg.carSitnum}" autocomplete="off" class="layui-input">
            </div>
        </div>
        <div class="layui-form-item">
            <label class="layui-form-label">车型数量</label>
            <div class="layui-input-block">
                <input type="number" name="carNumber" value="${carMsg.carNumber}" autocomplete="off" class="layui-input">
            </div>
        </div>
        ...
        <div class="layui-form-item">
            ...
            <div class="layui-input-inline">
                <select name="carType" id="carType">
                    <option
                            <c:if test="${carMsg.carType.equals('手动挡')}">
                                selected="selected"
                            </c:if>
                            value="手动挡" >手动挡
                    </option>
                    <option
                            <c:if test="${carMsg.carType.equals('自动挡')}">
                                selected="selected"
                            </c:if>
                            value="自动挡" >自动挡
                    </option>
                    <option
                            <c:if test="${carMsg.carType.equals('手自一体')}">
                                selected="selected"
                            </c:if>
                            value="手自一体" >手自一体
                    </option>
                    <option
                            <c:if test="${carMsg.carType.equals('自排')}">
                                selected="selected"
                            </c:if>
                            value="自排" >自排
                    </option>
                    <option
                            <c:if test="${carMsg.carType.equals('无级变速')}">
                                selected="selected"
                            </c:if>
                            value="无级变速" >无级变速
                    </option>
            ...
            <div class="layui-input-inline">
                <select name="cityId" id="cityId">
                    <c:forEach begin="0" end="${cityAll.size() - 1}" var="i">
                        <option
                                <c:if test="${carMsg.cityId == cityAll[i].cityId}">
                                    selected="selected"
                                </c:if>
                                value="${cityAll[i].cityId}">${cityAll[i].cityName}</option>
                    </c:forEach>
                </select>
            </div>
        </div>
